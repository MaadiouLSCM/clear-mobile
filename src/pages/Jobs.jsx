...
const FILTERS = ['ALL', 'RFC', 'COLLECTION', 'IN_TRANSIT', 'AT_HUB', 'DELIVERED', 'CLOSED'];
...
export default function Jobs() {
  ...
  useEffect(() => {
    api('/jobs').then(d => setJobs(safeArray(d))).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const filtered = jobs.filter(j => {
    ...
    if (search) {
      ...
      if (!jobRef(j).toLowerCase().includes(q) && !jobClient(j).toLowerCase().includes(q) && !(j.poNumber || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div style={{ padding: '16px 16px 24px' }}>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  JOB-001, SNEPCO…" style={{
        width: '100%', padding: '12px 14px', marginBottom: 12, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, color: C.text, fontSize: 14, outline: 'none',
      }} />
       ...
       filtered.map(job => {
        ...
        return (
          <div key={job.id} onClick={() => navigate(`/jobs/${job.id}`)} style={{
            background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14, marginBottom: 8, cursor: 'pointer',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div style={{ fontFamily: FONTS.mono, fontWeight: 700, fontSize: 14, color: C.gold }}>{jobRef(job)}</div>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: st.bg, color: st.color }}>{st.label}</span>
            </div>
            <div style={{ display: 'flex', gap: 16, fontSize: 12, color: C.muted }}>
              <span>👤 {jobClient(job)}</span>
              <span>🛣️ {jobCorridor(job)}</span>
            </div>
            {job.poNumber && <div style={{ fontSize: 11, color: C.mid, marginTop: 4 }}>PO: {safeStr(job.poNumber)}</div>}
          </div>
        );
      })}
    </div>
