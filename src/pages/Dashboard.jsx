...
function KpiCard({ icon, label, value, color, bg, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ width: 40, height: 40, borderRadius: 10, background: bg || C.goldDim,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (!m) return;
      const jl = safeArray(jr.status === 'fulfilled' ? jr.value : []);
      const tl = safeArray(tr.status === 'fulfilled' ? tr.value : []);
      const dl = safeArray(dr.status === 'fulfilled' ? dr.value : []);
      const cl = safeArray(cr.status === 'fulfilled' ? cr.value : []);
      setJobs(jl.slice(0, 8));
      setStats({
        active: jl.filter(j => j && !['CLOSED','DELIVERED','ABORTED','INVOICED'].includes(j.status)).length,
        tasks: tl.filter(t => t && t.status !== 'DONE').length,
        docs: dl.filter(d => d && (d.status === 'PENDING' || d.status === 'DRAFT')).length,
        alerts: cl.filter(c => c && c.status === 'OPEN').length,
      });
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[{ icon: '📋', label: t('newRfc'), to: '/reports' },{ icon: '📷', label: t('scanQr'), to: '/scan' },{ icon: '📝', label: t('fieldReport'), to: '/reports' }].map(a => (
            <button key={a.label} onClick={() => navigate(a.to)} style={{
              ...
              padding: '12px 8px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, color: C.text,
            }}><span style={{ fontSize: 22 }}>{a.icon}</span><span style={{ fontSize: 11, fontWeight: 600 }}>{a.label}</span></button>
          ))}
        </div>
      </div>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <button onClick={() => navigate('/jobs')} style={{ background: 'none', border: 'none', color: C.gold, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{t('viewAll')} →</button>
        </div>
        {loading ? <div style={{ textAlign: 'center', padding: 32, color: C.muted }}>Loading...</div> : jobs.length === 0 ? <div style={{ textAlign: 'center', padding: 32, color: C.muted }}>No jobs</div> : jobs.map(j => {
          if (!j) return null;
          const st = STATUS_MAP[j.status] || { color: C.muted, bg: 'rgba(107,114,128,0.12)', label: safeStr(j.status) };
          return (
            <div key={j.id} onClick={() => navigate(`/jobs/${j.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: C.gold, fontFamily: FONTS.mono }}>{jobRef(j)}</div>
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{jobClient(j)} · {jobCorridor(j)}</div>
              </div>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '4px 8px', borderRadius: 6, background: st.bg, color: st.color, whiteSpace: 'nowrap' }}>{st.label}</span>
            </div>
          );
        })}
      </div>
