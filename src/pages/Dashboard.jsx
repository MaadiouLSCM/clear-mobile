  ...
  return (
    <div style={{ padding: '16px 16px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
        ...
        <KpiCard icon="✓" label={t('myTasks')} value={loading ? '—' : stats.tasks} color={C.blue} bg={C.blueDim} onClick={() => nav('/tasks')} />
        <KpiCard icon="◫" label={t('docsToReview')} value={loading ? '—' : stats.docs} color={C.orange} bg={C.orangeDim} onClick={() => nav('/documents')} />
        <KpiCard icon="▲" label={t('alerts')} value={loading ? '—' : stats.alerts} color={C.red} bg={C.redDim} onClick={() => nav('/alerts')} />
      </div>
